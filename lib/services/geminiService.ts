...
class GeminiService {
  ...
  private hourlyRequestLimit: number;
  ...
  constructor() {
    ...
    this.dailyRequestLimit = parseInt(process.env.GEMINI_DAILY_LIMIT || '1000') || 1000;
    this.hourlyRequestLimit = parseInt(process.env.GEMINI_HOURLY_LIMIT || '100') || 100;
  ...
  private generatePrompt(resumeText: string, preferences: AnalysisPreferences): string {
    ...
    const config = roastConfigs[sanitizedPreferences.roastLevel as keyof typeof roastConfigs][sanitizedPreferences.language as keyof typeof roastConfigs.pyar];
...
8. ${sanitizedPreferences.roastLevel === 'dhang' ? 'Feel free to be brutally honest and use authentic cultural expressions for genuine feedback' : 'Maintain appropriate professional tone'}`;
  ...
  private generateCulturalContext(preferences: AnalysisPreferences): string {
    const contexts = {
    ...
    let cultural = contexts[preferences.language as keyof typeof contexts] || contexts.english;
  ...
  private sanitizePreferences(preferences: AnalysisPreferences): AnalysisPreferences {
  ...
  private detectSuspiciousContent(text: string, preferences: AnalysisPreferences): SecurityCheck {
  ...
  public async analyzeResume(resumeText: string, preferences: AnalysisPreferences) {
  ...
  private async makeRequestWithRetry(prompt: string, requestId: string, retryCount: number = 0): Promise<AnalysisResponse> {
    try {
      ...
      if (typeof error === 'object' && error !== null && 'message' in error) {
        ...
        errorStack = process.env.NODE_ENV !== 'production' && 'stack' in error ? String((error as { stack?: string }).stack) : undefined;
      }
  ...
  private parseAndValidateResponse(text: string, requestId: string): AnalysisResponse {
    try {
  ...
  private validateComprehensiveResponse(response: any): void {
  ...
  private sanitizeComprehensiveResponse(response: any): AnalysisResponse {
    ...
    const sanitized = {
      ...
      strengths: (response.strengths || [])
        .slice(0, 6)
        .map((s: any) => validator.escape(String(s)).substring(0, 200))
        .filter((s: string) => s.length > 0),
      weaknesses: (response.weaknesses || [])
        .slice(0, 5)
        .map((w: any) => validator.escape(String(w)).substring(0, 200))
        .filter((w: string) => w.length > 0),
      improvements: (response.improvements || [])
        .slice(0, 6)
        .map((imp: any) => ({
          priority: (['high', 'medium', 'low'].includes(imp.priority) ? imp.priority : 'medium') as 'high' | 'medium' | 'low',
        ...
        .filter((imp: Improvement) => imp.title.length > 0 && imp.description.length > 0),
      
      // Add proper extractedInfo sanitization
      extractedInfo: this.sanitizeExtractedInfo(response.extractedInfo || {}),
      
      // Add proper analytics sanitization
      resumeAnalytics: this.sanitizeResumeAnalytics(response.resumeAnalytics || {}),
      
      // Add contact validation sanitization
  ...
  // Add the missing sanitization methods
  private sanitizeExtractedInfo(extractedInfo: any): ExtractedInfo {
    ...
    return {
      ...
      skills: {
        technical: (extractedInfo.skills?.technical || []).slice(0, 50).map((skill: any) => 
          validator.escape(String(skill)).substring(0, 50)
        ),
        soft: (extractedInfo.skills?.soft || []).slice(0, 20).map((skill: any) => 
          validator.escape(String(skill)).substring(0, 50)
        ),
        languages: (extractedInfo.skills?.languages || []).slice(0, 10).map((lang: any) => 
          validator.escape(String(lang)).substring(0, 100)
        ),
        tools: (extractedInfo.skills?.tools || []).slice(0, 30).map((tool: any) => 
          validator.escape(String(tool)).substring(0, 50)
        ),
        frameworks: (extractedInfo.skills?.frameworks || []).slice(0, 30).map((framework: any) => 
          validator.escape(String(framework)).substring(0, 50)
        )
      },
      
      experience: (extractedInfo.experience || []).slice(0, 15).map((exp: any) => ({
        ...
        achievements: (exp.achievements || []).slice(0, 10).map((achievement: any) => 
          validator.escape(String(achievement)).substring(0, 300)
        ),
        technologies: (exp.technologies || []).slice(0, 20).map((tech: any) => 
          validator.escape(String(tech)).substring(0, 50)
        )
      })),
      
      education: (extractedInfo.education || []).slice(0, 10).map((edu: any) => ({
        ...
        honors: (edu.honors || []).slice(0, 5).map((honor: any) => 
          validator.escape(String(honor)).substring(0, 200)
        ),
        coursework: (edu.coursework || []).slice(0, 10).map((course: any) => 
