...
class FileProcessor {
  ...
  constructor() {
    ...
    this.maxFileSize = parseInt(process.env.MAX_FILE_SIZE || '5242880') || 5 * 1024 * 1024; // 5MB
    this.minFileSize = 100; // Minimum 100 bytes
    this.maxTextLength = 500000; // Maximum 500KB of text
    this.extractionTimeout = parseInt(process.env.EXTRACTION_TIMEOUT || '45000') || 45000; // 45 seconds
  ...
  async initializeProcessor(): Promise<void> {
    try {
  ...
  private validateMagicNumber(magicBytes: Buffer, expectedMimetype: string): boolean {
    try {
  ...
  async extractText(file: FileUpload): Promise<string> {
  ...
  private async performExtraction(file: FileUpload, extractionId: string): Promise<string> {
    try {
  ...
  private async extractFromPDF(file: FileUpload, extractionId: string): Promise<string> {
    try {
  ...
  private async extractFromDocx(file: FileUpload, extractionId: string): Promise<string> {
    try {
  ...
  private async extractFromDoc(file: FileUpload, extractionId: string): Promise<string> {
    try {
